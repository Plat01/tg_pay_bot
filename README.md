...
(`PaymentMethodsService`). Провайдер попадает в список, если:

1. заданы его учетные данные (`is_configured()`);
2. его API отвечает на проверочный запрос (`check_availability()`);
3. у него включен хотя бы один способ оплаты (`get_payment_methods()`).

Для Platega набор способов задается в `PLATEGA_ENABLED_METHODS` (коды через
запятую: `2` — СБП QR, `3` — ЕРИП, `11` — карта РФ, `12` — международная
карта, `13` — криптовалюта).

Если ни один провайдер недоступен, внешние способы оплаты не показываются:
остается только оплата с баланса, а админам при перезапуске приходит
предупреждение. Кнопки из старых сообщений проверяются повторно в момент
нажатия, поэтому оплата через отключенный провайдер не начнется.

Способ оплаты передается в callback data вместе с именем провайдера
(`payment_method:{provider}:{code}:{tariff_type}`), так что кнопки разных
платежных систем могут соседствовать в одной клавиатуре.
...
2. Реализовать `get_payment_methods()` (и при необходимости `is_configured()`
   и `check_availability()`) — кнопки появятся автоматически
