...
| `/payment_methods` | Проверка платежных провайдеров и доступных способов оплаты |
...
| PAYMENT_METHOD_PROBE_AMOUNT | Сумма пробной транзакции для проверки способа | Нет (100) |
...
(`PaymentMethodsService`). Проверка идет в два шага:

1. настроен ли провайдер — заданы ли учетные данные (`is_configured()` и
   `check_availability()`);
2. работает ли каждый отдельный способ оплаты (`check_method_availability()`).

Второй шаг нужен потому, что у мерчанта может быть включена только часть
...
рублей. Пробная транзакция не сохраняется в БД, не оплачивается и истекает на
стороне Platega сама.

Сумма пробы не должна быть ниже минимального платежа у провайдера — иначе
провалятся проверки всех способов и кнопок не останется.

**Проверка не отключает способ при технической ошибке.** Способ убирается
только если провайдер ответил клиентской ошибкой (4xx), то есть явно сказал,
что так платить нельзя. Сетевая ошибка, таймаут или 5xx оставляют способ на
месте и пишутся в лог: иначе временный сбой провайдера лишил бы бота всех
кнопок оплаты.
...
### Диагностика способов оплаты

Команда `/payment_methods` (только для админов) заново опрашивает провайдеры
и присылает результат по каждому способу: доступен он или отклонен и с каким
текстом ошибки от API. Там же показано, какие кнопки видит пользователь и
какой провайдер обслуживает каждую из них.

Это первое, что стоит запустить, если бот пишет «Нет доступных способов
оплаты».
